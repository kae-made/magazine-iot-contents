...
int sendDataToUart(const char* logName, const char* data)
{
...
static void tx_task(void* arg)
{
    ...
    while (1) {
        sendDataToUart(TX_TASK_TAG, "Hello world");
...
{
    void (*consumeMessage)(uint8_t * data, int length) = (void (*)(uint8_t * data, int length))arg;
    esp_log_level_set(RX_TASK_TAG, ESP_LOG_INFO);
    uint8_t* data = (uint8_t*)malloc(RX_BUF_SIZE + 1);

    ESP_LOGI(RX_TASK_TAG, "UART receive task started.");
    ...
    while (1) {
        ...
        if (rxBytes > 0) {
            ...
            consumeMessage(data, rxBytes);
        }
        else {
            ESP_LOGI(RX_TASK_TAG, "UART receive task received 0 bytes data.");
        }
    }
    free(data);

    ESP_LOGI(RX_TASK_TAG, "UART receive task finished.");
}

void initUart(void *consumeMessage) {
    ...
    ESP_LOGI(RX_TASK_TAG, "UART initialized");

    xTaskCreate(rx_task, "uart_rx_task", 1024 * 2, consumeMessage, configMAX_PRIORITIES, NULL);
