...
extern int sendDataToUart(const char* logName, const char* data);
                                                                                  ...
                                                                                  &pucComponentName, &ulComponentNameLength ) ) == eAzureIoTSuccess )
    {
                                                 ...
                                                 lengthof( sampleazureiotPROPERTY_TELEMETRY_FREQUENCY ) ) )
        {
            ...
            *pulWritablePropertyResponseBufferLength = prvGenerateAckForTelemetryFrequencyPropertyUpdate(
            ...
            const char uartSendBuf[1024];
            sprintf(uartSendBuf, "{\"interval\":%d}", lTelemetryFrequencySecs);
            sendDataToUart(TAG, uartSendBuf);
            ESP_LOGI(TAG, "Telemetry frequency sent to UART - '%s'.\r\n", uartSendBuf);
