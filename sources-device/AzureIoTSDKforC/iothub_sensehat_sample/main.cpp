...
static const char* connectionString = "HostName=<your-iot-hub>.azure-devices.net;DeviceId=<device-id>;SharedAccessKey=<SAS-key>";
...
void initializeSenseHat()
{
    ...
    printf("IMU for Sense Hat has been initialized.\n");
}
...
int main(void)
{
    ...
    char telemetry_msg_buffer[1024];

    int messagecount = 0;

    printf("Initializing IMU for Sense Hat...\n");
    initializeSenseHat();
    ...
    else
    {
        ...
        while(g_continueRunning)
        {
            ...
            printf("Sample rate %d: %s\n", messagecount, RTMath::displayDegrees("", imuData.fusionPose));
            ...
            printf(", fusionPose:%4.4f,%4.4f,%4.4f", imuData.fusionPose.x(), imuData.fusionPose.y(), imuData.fusionPose.z());
            printf(", accel:%4.4f,%4.4f,%4.4f", imuData.accel.x(), imuData.accel.y(), imuData.accel.z());
            printf(", gyro:%4.4f,%4.4f,%4.4f", imuData.gyro.x(), imuData.gyro.y(), imuData.gyro.z());
            printf(", compass:%4.4f,%4.4f,%4.4f\n", imuData.compass.x(), imuData.compass.y(), imuData.compass.z());
            ...
            sprintf(telemetry_msg_buffer, "{\"temperature\":%.3f,\"humidity\":%.3f,\"pressure\":%.3f},\"accelerometer\":{\"x\":%.4f,\"y\":%.4f,\"z\":%.4f},\"gyro\":{\"x\":%.4f,\"y\":%.4f,\"z\":%.4f},\"compass\":{\"x\":%.4f,\"y\":%.4f,\"z\":%.4f}}",
                imuData.temperature, imuData.humidity, imuData.pressure,
